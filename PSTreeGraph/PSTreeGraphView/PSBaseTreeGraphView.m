...
- (void) setSelectedModelNodes:(NSSet *)newSelectedModelNodes
{
    ...
    if (selectedModelNodes_ != newSelectedModelNodes) {
        ...
        // Show node from the same level
        [self hideNodesFromDifferentBranchIfNeeded: newSelectedModelNodes];
...
- (void) hideNodesFromDifferentBranchIfNeeded: (NSSet *) newModelNodes {
    id <PSTreeGraphModelNode> previousSelectedModelNode = self.singleSelectedModelNode;
    id <PSTreeGraphModelNode> newModelNode = [newModelNodes anyObject];
    ...
    if (!oldNodeOnSameBranchAsNewNode) {
        ...
        PSBaseSubtreeView *subtreeView = [self subtreeViewForModelNode:ancestorOfPreviousNodeFromSameLevel];
        if (subtreeView.expanded)
...
-(void) touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (hitModelNode) {
        PSBaseSubtreeView *subtreeView = [self subtreeViewForModelNode:hitModelNode];
        [subtreeView toggleExpansion:nil];
    } else {
        if (self.rootSubtreeView.expanded)
        [self.rootSubtreeView toggleExpansion:nil];
    }
